...
router.post("/:id/transcode", async (req, res) => {
  ...
  if (!secret || authorization !== `Bearer ${secret}`) return res.status(401).json({ error: "Non autorizzato" });
  const id = z.string().uuid().safeParse(req.params.id);
  if (!id.success) return res.status(400).json({ error: "ID video non valido" });
  const parsed = transcodeRequestSchema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ error: "Richiesta transcodifica non valida", details: parsed.error.flatten().fieldErrors });
  const queue = getTranscodeQueue();
  const job = await queue.add("transcode-hls", { videoId: id.data, ...parsed.data }, {
    jobId: `video-${id.data}`,
    attempts: 3,
  ...
  await prisma.video.update({ where: { id: id.data }, data: { processingStatus: "QUEUED", processingError: null } }).catch(() => undefined);
  return res.status(202).json({ ok: true, data: { jobId: job.id, videoId: id.data, queue: process.env.QUEUE_NAME ?? "tvmix-video-transcoding" } });
});
